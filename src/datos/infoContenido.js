...
export const INFO = {
  ...
  dqn: {
    ...
    dato: "Encoder conv 8×10 (16,32) + rama cinemática → 128→128→3 · replay 100.000 · lote 128 · ε 1.0→0.05 en 25.000 pasos · γ=0.99 · τ=0.01 · Double DQN + Huber · off-policy, basado en valor.",
  },
  ppo: {
    ...
    dato: "Actor conv(8×10)+cinemática→128→128→3 · crítica conv→128→128→1 · rollout 256 pasos/entorno · 4 épocas · minibatch 1024 · clip ε=0.2 · GAE λ=0.95 · coef. valor 0.5 · coef. entropía 0.01 · on-policy.",
  },
  sac: {
    ...
    dato: "Actor + 2 críticos + 2 redes objetivo, todos con encoder conv(8×10)+rama cinemática→128→128→3 · replay 100.000 · lote 128 · α aprendible (arranca en 0.20) · entropía objetivo 0.55·log(3) · τ=0.01 · off-policy, máxima entropía.",
  },
  worldModel: {
    ...
    dato: "Q-net conv(8×10)+cinemática→128→128→3 (Double DQN) · modelo de dinámica (estado+acción)→200→200→(Δ, r, done) · replay 100.000 · planning 5 imaginadas/paso real · arranque modelo 1.000, Q-net 2.000 · ε 1.0→0.05 · off-policy, model-based.",
  },
  ...
  worldModelRecurrente: {
    ...
    secciones: [
      ...
      { h: "De dónde viene", cuerpo: "La idea está tomada de <b>«World Models»</b> (Ha &amp; Schmidhuber, 2018), donde un agente aprende un VAE que comprime la imagen, una memoria recurrente (MDN-RNN, un LSTM) que predice cómo evoluciona esa imagen comprimida, y un pequeño controlador. Aquí no hace falta el VAE —nuestro estado son números (6 cinemáticos + la matriz de ocupación 8×10), no una imagen de píxeles— así que tomamos solo la pieza valiosa para nosotros: la <b>memoria recurrente</b> como modelo de dinámica. Usamos una versión simplificada que predice el estado directamente, sin la mezcla de gaussianas del MDN-RNN original." },
    ...
    dato: "Q-net conv(8×10)+cinemática→128→128→3 (Double DQN) · dinámica LSTM: (estado+acción)→128 (estado oculto)→(Δs, r, done) · entrena secuencias de 16 pasos, 32 por lote · buffer de 256 episodios · planning 5 imaginadas/paso real · ε 1.0→0.05 · off-policy, model-based recurrente.",
  },
  ...
  modelFree: { emoji:"🚀", titulo:"Model-free", categoria:"Concepto",
    ...
    secciones:[
      ...
      {h:"En este laboratorio",cuerpo:"DQN, PPO y SAC son model-free. DQN, por ejemplo, aprende los valores <span class=\"mono\">Q(s,a)</span> probando acciones y observando recompensas, sin saber jamás cómo rebota la pelota ni dónde están los ladrillos; lo único que llega a la red es el estado (6 cinemáticos + la matriz de ocupación 8×10 de los ladrillos) y la recompensa de cada paso."}] },
  ...
  modeloDinamica: { emoji:"🔭", titulo:"Modelo de dinámica", categoria:"Concepto",
    ...
    secciones:[
      ...
      {h:"En este laboratorio",cuerpo:"Lo usa el World Model. Su entrada es el estado (6 cinemáticos + matriz de ocupación 8×10) junto con la acción codificada, y su salida son los incrementos del estado, la recompensa y un indicador de fin de episodio. Su calidad se vigila con el «error del modelo» (RMSE): mientras sea bajo, las partidas imaginadas son fiables."}],
  ...
  rewardMedio100: { emoji:"🏆", titulo:"Recompensa media (·100)", categoria:"Métrica",
    resumen:"La métrica principal: el promedio de puntos por partida en las últimas 100. Si sube de forma sostenida, el agente está aprendiendo.",
    secciones:[
      ...
      {h:"Cómo se calcula",cuerpo:"En cada episodio se suman todos los premios y castigos, paso a paso, hasta que termina; esa suma es el <b>retorno</b> de la partida (<span class=\"mono\">G = r₁ + r₂ + … + r_T</span>). Los componentes aquí son <span class=\"mono\">+1</span> por ladrillo (con bonus de combo), <span class=\"mono\">+0.2</span> por rebote con la pala, <span class=\"mono\">−1</span> por perder la pelota y <span class=\"mono\">+5</span> por completar el nivel. La métrica es la <b>media móvil</b> del retorno sobre las 100 partidas más recientes. El «·100» del nombre solo indica que en la <i>curva</i> se dibuja multiplicada por 100 para compartir eje con la tasa de éxito; el número del panel es el valor real."},
      {h:"La teoría: es el objetivo del RL",cuerpo:"No es una métrica cualquiera: <b>es exactamente lo que el agente intenta maximizar</b>. La meta de todo el aprendizaje por refuerzo es maximizar el <b>retorno esperado</b>, <span class=\"mono\">J(π) = E[ Σ γᵗ·rₜ ]</span> —la suma de recompensas que cabe esperar siguiendo la política <span class=\"mono\">π</span>—. La recompensa media de los últimos episodios es, precisamente, la estimación empírica de ese objetivo. Por eso es <i>la</i> curva a vigilar: todo lo demás (pérdida, ε, entropía…) son medios; esto es el fin."},
      {h:"Cómo leerla",cuerpo:"Fíjate en la <b>tendencia</b>, no en el punto a punto: el aprendizaje por refuerzo es ruidoso y la cifra oscila aun cuando el agente mejora. Al principio suele ser <b>negativa</b> (pierde la pelota una y otra vez, cada pérdida resta 1); se vuelve <b>positiva</b> cuando aprende a devolverla y a romper ladrillos, y crece más si llega a completar niveles."}],
    formula:"recompensa media = (1/N)·Σᵢ Gᵢ   sobre los últimos N=100 episodios,   con Gᵢ = Σₜ rₜ del episodio i",
    ejemplo:"Si en las últimas 100 partidas el agente rompió de media 12 ladrillos (+12), dio unos 8 rebotes (+1.6) y acabó perdiendo la bola (−1), su retorno medio rondaría <b>+12.6</b>. Ese es el número que verías en el panel." },
  loss: { emoji:"📉", titulo:"Pérdida", categoria:"Métrica",
    resumen:"El error de la red respecto a su objetivo en cada actualización; el optimizador lo reduce ajustando los pesos.",
    secciones:[
      ...
      {h:"Cómo se calcula",cuerpo:"En los métodos basados en valor (DQN, World Model) es el <b>error medio entre la predicción y el objetivo de Bellman</b> sobre el lote: se compara <span class=\"mono\">Q(s,a)</span> con <span class=\"mono\">r + γ·(1−done)·Q⁻(s')</span> y se promedia con la <b>pérdida de Huber</b> (cuadrática para errores pequeños, lineal para los grandes, de modo que un dato muy raro no la dispare). En PPO es la suma del objetivo recortado del actor, el error de la crítica y un término de entropía; en SAC, el error de sus dos críticos. El optimizador (Adam) usa el gradiente de esta pérdida para corregir los pesos."},
      {h:"Cómo leerla",cuerpo:"A diferencia del aprendizaje supervisado, en RL la pérdida <b>no tiene por qué bajar siempre</b>. El objetivo se mueve junto con la propia red (persigue una diana que ella misma desplaza), así que puede subir y bajar sin que eso indique nada malo. Lo verdaderamente importante es que <b>no diverja</b>: que no se dispare hacia valores enormes, señal de inestabilidad."}],
    formula:"L = media_lote[ Huber( Q(s,a) − ( r + γ·(1−done)·Q⁻(s', argmaxₐ' Q(s',a')) ) ) ]   (caso DQN)" },
  ...
  tasaExito100: { emoji:"🥇", titulo:"Tasa de éxito", categoria:"Métrica",
    ...
    secciones:[
      {h:"Qué mide",cuerpo:"El porcentaje de los <b>últimos 100 episodios</b> en que el agente <b>completó el nivel</b>, es decir, rompió los 80 ladrillos (8 filas × 10 columnas) antes de perder la pelota o agotar el tiempo."},
    ...
    formula:"tasa de éxito = (niveles completados en los últimos 100 episodios) / 100 · 100 %" },
  entropia: { emoji:"🌪️", titulo:"Entropía de la política", categoria:"Métrica",
    ...
    formula:"H(π) = −Σₐ π(a|s)·log π(a|s)   ·   = 0 si siempre la misma acción;   = log(3) ≈ 1.10 si las tres son equiprobables",
  ...
  lab: { emoji:"🧠", titulo:"Arkanoid DRL Learning Lab", categoria:"Interfaz",
    resumen:"Un laboratorio para ver, en directo, cómo cinco algoritmos de IA aprenden a jugar al Arkanoid desde cero.",
    secciones:[
      {h:"Qué es esto",cuerpo:"Un laboratorio donde cinco algoritmos de <b>aprendizaje por refuerzo profundo</b> (Deep RL) aprenden a jugar al Arkanoid <b>de verdad</b>: con redes neuronales reales que se entrenan en la GPU de tu navegador, paso a paso, ante tus ojos. A ninguno se le explican las reglas. No hay guion ni trampa: descubren cómo se juega a base de jugar miles de partidas y ajustar sus redes con lo que observan. Todo lo que ves —la partida, las métricas, las curvas, el inspector— sale de ese entrenamiento ocurriendo en tiempo real."},
      {h:"El juego: Arkanoid",cuerpo:"Arkanoid (primo del Breakout) es el clásico de la pala y la pelota. El agente controla una <b>pala</b> abajo y solo puede hacer tres cosas: moverla a la <b>izquierda</b>, <b>mantenerla</b> quieta o moverla a la <b>derecha</b>. La <b>pelota</b> rebota sola; si toca un <b>ladrillo</b>, lo rompe; si la pala la golpea, vuelve a subir; si se escapa por abajo, se pierde y el episodio (la partida) termina. El objetivo es <b>romper todos los ladrillos</b> sin que la pelota se cuele. También se acaba la partida si se agota el tiempo (un límite de pasos proporcional al nº de ladrillos)."},
      {h:"La restricción de los ladrillos (lo difícil)",cuerpo:"Aquí está la clave de todo el proyecto. El tablero tiene <b>80 ladrillos</b> en una rejilla de <b>8 filas × 10 columnas</b>. Para <b>apuntar</b> —dirigir la pelota hacia donde aún quedan ladrillos— al agente no le basta con saber dónde está la pelota: necesita saber <b>qué ladrillos siguen vivos y dónde</b>. Por eso su «vista» del juego incluye una <b>matriz de ocupación 8×10</b> (un 1 por cada ladrillo intacto, un 0 por cada hueco), que una red <b>convolucional</b> lee aprovechando su estructura espacial, como un pequeño mapa. Sin esa visión el agente juega a ciegas y se estanca; con ella puede planear. (Se conserva una versión <b>ciega</b> —solo posición de pelota y pala, sin ladrillos— como contraste, para medir cuánto aporta la visión.)"},
      {h:"El bucle de aprendizaje",cuerpo:"Todo gira en torno a un mismo ciclo, el corazón del aprendizaje por refuerzo. El agente observa el <b>estado</b> (la situación del juego), elige una <b>acción</b> (mover la pala), el entorno responde con una <b>recompensa</b> (premio o castigo) y un <b>nuevo estado</b>, y el agente usa esa señal para ajustar su forma de jugar —su <b>política</b>—. Repetido millones de veces, ese ciclo es, literalmente, lo que llamamos aprender. Cada vuelta produce una <b>transición</b> <span class=\"mono\">(s, a, r, s', done)</span>, el ladrillo elemental con el que se construye todo."},
      {h:"Los cinco algoritmos",cuerpo:"Cada uno resuelve el mismo juego con una filosofía distinta:<br>• <b>DQN</b> — aprende cuánto vale cada acción (su valor Q) y elige la mejor.<br>• <b>PPO</b> — ajusta directamente la política, a pasos pequeños y controlados (actor-crítico, on-policy).<br>• <b>SAC</b> — busca recompensa <i>y</i> mantener variedad en sus decisiones; se autorregula (máxima entropía).<br>• <b>World Model</b> — aprende un modelo del juego y entrena también con partidas <i>imaginadas</i> (Dyna-Q).<br>• <b>World Model RNN</b> — igual, pero su modelo tiene <i>memoria</i> (un LSTM que aprende secuencias).<br>Cada tarjeta de algoritmo tiene su propio icono ℹ con la explicación a fondo."},
      {h:"Los botones y mandos",cuerpo:"Arriba a la derecha: <b>▶ Entrenar</b> arranca o pausa el entrenamiento continuo (el agente juega y aprende sin parar). <b>⏭ Paso</b> avanza un único lote de entrenamiento, para examinar el aprendizaje «a cámara lenta». <b>↻ Reiniciar</b> borra todo lo aprendido y empieza el agente de cero. Más abajo, en <b>Control de ejecución</b>: la <b>velocidad</b> (avance rápido; no cambia la física), cuántos <b>entornos</b> generan datos y cuántos se dibujan, y el <b>reward shaping</b> (una pista extra de recompensa). Arriba del todo, el selector elige cuál de los cinco algoritmos pilota al agente."},
      {h:"Cómo sacarle partido",cuerpo:"Elige un algoritmo, pulsa <b>Entrenar</b> y observa: la <b>recompensa media</b> debería subir con el tiempo (esa es la señal de que aprende), la partida observada jugará cada vez mejor y el <b>inspector</b> te deja asomarte a lo que «piensa» la red. Pincha en cualquier icono <b>ℹ</b> para entender a fondo cada pieza —están pensados para recorrerse en orden—. Y en la pestaña <b>Comparativa de modelos</b> puedes enfrentar los cinco algoritmos sobre el mismo problema."}],
    formula:"estado → acción → recompensa → estado siguiente → (ajustar política y vuelta a empezar)",
    dato:"Estado de 86 números (6 cinemáticos + matriz de ocupación 8×10) · 3 acciones (←, ·, →) · 80 ladrillos · recompensa +1 ladrillo / +0.2 rebote / −1 perder / +5 nivel · 5 algoritmos · redes en GPU (WebGPU/WebGL) con TensorFlow.js." },
  ...
  arenaJugar: { emoji:"🎮", titulo:"Jugar — modelos entrenados", categoria:"Interfaz",
    resumen:"Pon a jugar a los modelos ya entrenados y verifica que se pasan niveles.",
    secciones:[
      {h:"Qué es",cuerpo:"A diferencia del Laboratorio (donde ves <b>cómo</b> se entrena, desde cero) y de la Comparativa (un benchmark resumido), esta pestaña pone a jugar a los modelos <b>ya entrenados</b> y guardados —el «zoo»— en modo <b>greedy</b> (siempre la mejor acción, sin exploración). Es la prueba visual de que el sistema aprendió a jugar: rompe ladrillos y completa niveles."},
      {h:"Niveles que no vio",cuerpo:"Los modelos juegan sobre niveles del conjunto de <b>test</b> —patrones de ladrillos que <b>no</b> aparecieron en su entrenamiento—, así que lo que ves mide <b>generalización</b>, no memorización. El marcador cuenta niveles superados, racha y tasa de éxito de la sesión."},
      {h:"De dónde salen los modelos",cuerpo:"Se entrenan <b>una vez</b>, offline y a fondo, con <span class=\"mono\">npm run zoo</span> (arquitectura convolucional 8×10, la misma de las gráficas) y se guardan como ficheros versionados que la app carga al instante. El botón <b>Regenerar</b> reentrena un modelo aquí mismo, en el navegador, y guarda tu copia local sin tocar los oficiales."},
      {h:"Un modelo o los cinco",cuerpo:"Puedes ver un modelo grande con su marcador detallado, o los cinco en paralelo para compararlos jugando a la vez sobre niveles equivalentes."}] },
  ...
  arenaVelocidad: { emoji:"⏱️", titulo:"Velocidad de juego", categoria:"Control",
    resumen:"Ver al modelo jugar a cámara lenta o rápida, sin tocar la física.",
    secciones:[
      {h:"Qué hace",cuerpo:"Ajusta cuántos pasos de simulación se muestran por fotograma. Por encima de <span class=\"mono\">1×</span> es avance rápido (más pasos por segundo); por debajo (<span class=\"mono\">0.5×</span>, <span class=\"mono\">0.25×</span>) es cámara lenta: se da un paso cada varios fotogramas para seguir la jugada con detalle."},
      {h:"Lo que NO cambia",cuerpo:"Igual que en el Laboratorio, <b>no altera la física ni el modelo</b>: la pelota y la pala se mueven exactamente igual en cada paso. Solo cambia el ritmo al que ves la partida. La política es siempre la misma, greedy (sin exploración)."}] },
  ...
  estado: { emoji:"👁️", titulo:"Estado (s)", categoria:"Concepto",
    ...
    secciones:[
      {h:"Qué es",cuerpo:"El <b>estado</b> es la «foto» del juego que recibe la red en cada paso: <b>6 valores cinemáticos</b> (posición x/y de la pelota, su velocidad vx/vy, la posición de la pala y la distancia pelota–pala) <b>más la matriz de ocupación 8×10</b> de los 80 ladrillos (1 = vivo, 0 = roto). En total, 86 números normalizados ~[-1, 1]."},
      {h:"Cinemática + visión",cuerpo:"Los 6 cinemáticos dicen dónde está la pelota y la pala; la <b>matriz de ocupación 8×10</b> le dice qué ladrillos quedan y dónde, para poder <b>apuntar</b>. No son píxeles: es una rejilla estructurada de 0/1 que un <b>encoder convolucional</b> lee aprovechando su estructura espacial. (La baseline <b>ciega</b> —solo los 6 cinemáticos— se conserva como contraste pedagógico.)"},
      {h:"En el panel",cuerpo:"En la tira de transición se muestra un resumen legible (por ejemplo, las coordenadas de la pelota), pero lo que entra de verdad en la red es el estado completo: los 6 cinemáticos más la matriz de ocupación 8×10."}] },
  ...
  estadoSiguiente: { emoji:"➡️", titulo:"Estado siguiente (s')", categoria:"Concepto",
    ...
    secciones:[
      {h:"Qué es",cuerpo:"<span class=\"mono\">done</span> vale verdadero si el episodio terminó en este paso. Aquí ocurre por tres motivos: se perdió la pelota (cae por abajo), se limpió el nivel (se rompieron los 80 ladrillos) o se agotó el tiempo (el límite de pasos por episodio, proporcional al nº de ladrillos: 90 por ladrillo → 7200 en la rejilla 8×10)."},
